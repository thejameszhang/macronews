#!/bin/bash
# Dispatch src/kg/disambiguate.py to a FREE-tier B200 (gpu_b200/pi_btk22).
...
#   bash scripts/run_disambig.sh
#
# Optional env: THRESHOLD, SELF_REF_THRESHOLD, MODEL_PATH, PARTITION, ACCOUNT, GRES, WALLTIME, MEM,
#               EXCLUDE_NODES.
set -euo pipefail
...
CLUSTERS="${CLUSTERS:-}"
THRESHOLD="${THRESHOLD:-}"
PARTITION="${PARTITION:-gpu_b200}"; ACCOUNT="${ACCOUNT:-pi_btk22}"
GRES="${GRES:-gpu:b200:1}"; WALLTIME="${WALLTIME:-01:00:00}"; MEM="${MEM:-64G}"
...
jobid=$(sbatch --parsable \
    ...
    --wrap "module load Python/3.12.3-GCCcore-13.3.0 && \
        PYTHONPATH=src .venv/bin/python -m src.kg.disambiguate ${INPUT} \
            --output ${OUTPUT} ${CLUSTERS_FLAG} ${THRESH_FLAG} \
            ${SELF_REF_THRESHOLD:+--self-ref-threshold ${SELF_REF_THRESHOLD}} \
            ${MODEL_FLAG}")
echo "Submitted disambig job ${jobid} (partition ${PARTITION}, account ${ACCOUNT}, gres ${GRES})"

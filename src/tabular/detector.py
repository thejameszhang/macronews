...
Per-line classification rule (combined):
  ...
  A line is TABULAR iff
      (a) it is a pure separator line (only =, -, :, _, |, +, *, ~, .
          and whitespace — no word characters), OR
      (b) it has >= MIN_MID_LINE_GAPS multi-space gaps  (clear column structure)
   OR (c) it has >= 1 multi-space gap AND >= NUMERIC_TOKEN_FRAC of its tokens
          look numeric  (tight short-row tables like "Dec 72.45 up  0.03"
          where each row has only one column gap)

Otherwise the line is NARRATIVE.

Aggregation of these classifications across all <pre> blocks drives the
tabular_body filter (see src/loaders.py).

...
Yields one NMLArticle per <doc>...</doc> block in the file. We don't use
xml.etree because the raw archive is 171 GB across 1,260 monthly files
and full-tree parsing per file is wasteful — we only need:
...
The parser is line-buffered: it reads the file lazily, holds at most one
<doc> in memory at a time, and yields per article.

Trailer recognition mirrors `_WIRE_MARKER_ONLY_RE` in src/loaders.py:477,
plus ISO-timestamp-only <p>s.
"""
...
# ---------------------------------------------------------------------------
# Trailer detection — match the rules used in src/loaders.py.
# ---------------------------------------------------------------------------

# Wire markers: matches src/loaders.py:477 _WIRE_MARKER_ONLY_RE.

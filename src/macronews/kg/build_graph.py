"""Build a NetworkX MultiDiGraph KG from KG-extractor TemporalEvent JSONL output.

Nodes and edges are merged case-insensitively; the first-seen surface form
(title-cased, acronyms preserved) becomes the canonical node ID. `entity_groups_path`,
if given, adds synthetic ASSET_GROUP_NODE_TYPE anchor nodes so entities link to the
asset groups.
"""
...
from macronews.kg.schemas import ASSET_GROUP_NODE_TYPE, ASSET_GROUP_RELATION
...
def _group_display_names() -> dict[str, str]:
    ...
    from macronews.utils.groups import load_group_universe  # deferred: skip utils load when entity-groups unused

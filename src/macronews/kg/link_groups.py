...
def _judge_inputs(by_key: dict[str, EntityLinks], cfg_by_key: dict[str, GroupConfig],
    ...
    for cf, el in by_key.items():
        if not el.residual_keys:
            continue
        from macronews.kg.group_judge import GroupJudgeInput      # lazy: Phase 2 only (fires only when residual exists)
        cands = []
...
def link_entities(disambig_path: Path | str, mapper_path: Path | str | None,
    ...
    if use_llm:
        ...
        if items:
            from macronews.kg.group_judge import LLMGroupJudge   # lazy: Phase 2 only
...
def main() -> None:
    ...
    p.add_argument("--model", type=str, default=str(MAPPER_MODEL),
                   help="Residual judge (Phase 2 -- kg/group_judge.py is NOT YET WRITTEN)")

...
import macronews.compat as compat  # noqa: F401
import numpy as np
from pydantic import BaseModel

from macronews import invariants                       # main() applies them before vLLM loads
from macronews.config.paths import GRADER_MODEL
...
class InvalidationAgent:
    ...
    def _init_llm(self):
        if self._llm is None:
            ...
            # crashes on B200. Same pin as kg/grading/llm.py.
...
def main() -> None:
    ...
    p.add_argument("--model", type=str, default=str(GRADER_MODEL),
                   help="Supersession judge (default: QwQ-32B)")
    ...
    if not args.inspect_candidates and not args.out:

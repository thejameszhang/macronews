...
def _load_source(args) -> dict[str, dict]:
    if args.dataset == "gold":
        ...
        arts = load_articles(dataset="sports", sample_dir=args.sports_dir,
           ...
           # conversation past max_model_len (mirrors mapping/grading/runner.py).
           # An over-long article's statements then skip as
...
def main():
    ...
    p.add_argument("--model", type=str, default=str(GRADER_MODEL),
                   help="Judge model (default: QwQ-32B)")
    ...
    grader = LLMKGGrader(model_path=args.model, max_model_len=args.max_model_len,
                         tensor_parallel_size=invariants.TENSOR_PARALLEL_SIZE)

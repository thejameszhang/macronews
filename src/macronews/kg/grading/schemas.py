...
Schema-BLIND by design: the grader is never handed the entity/relation taxonomy, so
it judges from an outside reader's point of view. Field order on KGStatementVerdict
IS the chain of thought (read -> macro-relevance -> supported -> asserts_direction ->
per-triplet faithfulness) -- reordering the schema reorders the reasoning.
`asserts_direction` is judged from the statement's wording alone, not the relation
code, so the directional slice stays model-agnostic. See each Field's description
for what makes a verdict true or false.
"""

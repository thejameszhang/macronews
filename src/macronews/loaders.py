...
from macronews.config.paths import DJNW_ARTICLES_DIR, RESULTS_DIR

logger = logging.getLogger(__name__)

# Tabular-body sidecar (one .jsonl per YYYY-MM, generated by src/macronews/tabular/).
# Each row: {accession_number, p_tokens, pre_tabular_tokens, pre_narrative_tokens, is_tabular_body}.
_TABULAR_SIDECAR_DIR = RESULTS_DIR / "tabular"
...
def load_djnw_articles(
    ...
    # A missing MONTH is fine -- 200 of the corpus's 555 months have no sidecar yet.
    # A missing DIRECTORY is not: it means the path is wrong, and the loop below would
    # treat every month as "legitimately absent", silently no-op the tabular-body
    # filter, and map a different corpus with exit status 0.
    if not _TABULAR_SIDECAR_DIR.is_dir():
        raise FileNotFoundError(
            f"tabular sidecar directory not found: {_TABULAR_SIDECAR_DIR}\n"
            f"This is a WRONG PATH, not missing data. Individual months may "
            f"legitimately lack a sidecar, but the directory itself must exist -- "
            f"otherwise the tabular-body filter silently no-ops and the corpus "
            f"changes. Run the tabular stage first: macronews tabular --nml-file ..."
        )
    for fp in files:
    ...
    #                           src/macronews/tabular/, joined by accession_number)
    ...
    # zero narrative false positives).
